            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" action="{{route('taxes.update', [$item->id])}}" enctype="multipart/form-data">
                            @csrf
                            @method("patch")
                            ...
                            <div class="form-group">
                                ...
                                <select id="item_rate" name="taxrate_id" class="form-control">
                                    @foreach($taxrates as $id => $name)
                                        <option value="{{ $id }}" @if(old("taxrate_id", $item->taxrate_id) == $id) selected="selected" @endif>{{ $name }}</option>
                                    @endforeach
                                </select>
                                                    ...
                                                    <optgroup label="{{$state->name}}">
                                                        @if($state->counties)
                                                            @foreach($state->counties as $county)
                                                                <option value="{{ $county->id }}" @if(old("county_id", $item->county_id) == $county->id) selected="selected" @endif>{{ $county->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </optgroup>
                                                @endforeach
