            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" action="{{route('counties.update', [$item->id])}}" enctype="multipart/form-data">
                            @csrf
                            @method("patch")
                                        ...
                                        <optgroup label="{{$country->name}}">
                                            @if($country->states)
                                                @foreach($country->states as $state)
                                                    <option value="{{ $state->id }}" @if(old("state_id", $item->state_id) == $state->id) selected="selected" @endif>{{ $state->name }}</option>
                                                @endforeach
                                            @endif
                                        </optgroup>
                                    @endforeach
