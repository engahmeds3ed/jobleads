...
class CreateAfterTaxesUpdateTrigger extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::unprepared("
CREATE TRIGGER after_taxes_update
    ...
    BEGIN
	    UPDATE states INNER JOIN (
			...
			WHERE counties.id = NEW.county_id OR counties.id = OLD.county_id
		...
		UPDATE counties INNER JOIN (
			SELECT AVG(taxrates.amount) AS taxrates_avg, county_id FROM taxes
			INNER JOIN counties ON taxes.county_id = counties.id
    ...
    {
        DB::unprepared('DROP TRIGGER `after_taxes_update`');
    }
}
